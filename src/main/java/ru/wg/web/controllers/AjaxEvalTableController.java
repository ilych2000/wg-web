...
public class AjaxEvalTableController extends AbstractAjaxController {
	...
	private static final Logger LOG = Logger
			.getLogger(AjaxEvalTableController.class);
	...
	public Object doPostAjaxOperation(Object inObject,
		...
		if (isRaschet) {
			List<CalculationTable> calculateTable = _sqlManager
					.getCalculateTableListByID(calcID, true);
			...
			for (CalculationTable curCT : _sqlManager
					.getCalculateTableListByID(calcID, false)) {
				curentMapCT.put(
						curCT.getSCENARIOS_ID() + "-" + curCT.getCRITERIA_ID(),
						curCT);
			}

			for (CalculationTable genCT : _sqlManager
					.getGeneratedCalculateTableListByID(calcID)) {

				String key = genCT.getSCENARIOS_ID() + "-"
						+ genCT.getCRITERIA_ID();
				CalculationTable ct = curentMapCT.get(key);
				if (ct != null) {
					if (ct.getENABLE() != 1) {
						_sqlManager.setCalculateTableEnableByID(ct.getID(),
								true);
					}
