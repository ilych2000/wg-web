...
import ru.wg.web.pojo.Uzel;
import ru.wg.web.pojo.WGObject;

public class SQLiteManagerDaoJdbc implements ManagerDao {

    @SuppressWarnings("unused")
    private static final Logger LOG = Logger.getLogger(SQLiteManagerDaoJdbc.class);
    ...
    private <T> List<T> getResultListObject(Class<T> returnClass, String aSqltext,
            Object... aParams) throws Exception {

        List<Map<String, Object>> res =
                localDS.executeQueryListResult(DB_CONTEXT, aSqltext, aParams);
    ...
    @SuppressWarnings("unused")
    private <T> T getResultObject(Class<T> returnClass, String aSqltext, Object... aParams)
            throws Exception {
    ...
    public List<AreaControl> getControlList() throws Exception {
        ...
        List<AreaControl> ret =
                getResultListObject(AreaControl.class, "SELECT ID, NAME FROM CONTROL");

        for (AreaControl aControl : ret) {
            List<Area> areas = getResultListObject(Area.class,
                    "SELECT ID, NAME FROM AREA WHERE CONTROL_ID=?", aControl.getID());
            aControl.setAREAS(areas);
            for (Area area : areas) {
                List<Uzel> uzels = getResultListObject(Uzel.class,
                        "SELECT ID, NAME FROM UZEL WHERE AREA_ID=?", area.getID());
                area.setUZELS(uzels);

                for (Uzel uzel : uzels) {
                    List<WGObject> wgObjects = getResultListObject(WGObject.class,
    ...
    public List<Calculation> getCalculateListByObjectID(Integer aID) throws Exception {
        ...
        List<Calculation> ret = getResultListObject(Calculation.class,
    ...
    public List<Scenarios> getScenariosListByCalculateID(Integer aID) throws Exception {
        ...
        List<Scenarios> ret = getResultListObject(Scenarios.class,
    ...
    public List<CalculationCriteria> getCriteriaListByCalculateID(Integer aID) throws Exception {
        ...
        List<CalculationCriteria> ret = getResultListObject(CalculationCriteria.class,
    ...
    @Override
    public Map<String, Integer> getScenariosCriteriaListByCalculateID(Integer aID)
            throws Exception {

        List<ScenariosCriteria> res = getResultListObject(ScenariosCriteria.class,
        ...
        for (ScenariosCriteria sc : res) {
            ret.put(sc.getSCENARIOS_ID() + "-" + sc.getCRITERIA_ID(), sc.getCHECKING());
        }
    ...
    public void setScenariosCriteriaCheck(Integer aCalculateID, Integer aScenarioID,
        ...
        List<Map<String, Object>> res = localDS.executeQueryListResult(DB_CONTEXT,
        ...
        localDS.executeUpdate(DB_CONTEXT, sql, aCheckValue, aCalculateID, aScenarioID, aCriteriaID);
    ...
    public void updateFieldValue(String aDataTable, String aDataField, String aDataValue,
        ...
        localDS.executeUpdate(DB_CONTEXT,
                "UPDATE " + aDataTable + " SET " + aDataField + "=? WHERE ID=?", aDataValue,
                aDataId);
    }
    ...
    public List<ScenarioGroup> getScenarioGroupList() throws Exception {
        ...
        List<ScenarioGroup> ret =
                getResultListObject(ScenarioGroup.class, "SELECT * FROM SCENARIOS_GROUP");

        return ret;
    }

    @Override
    public void setCalculationScenariosIdsCheck(Integer calcID, Integer scenarioID, Integer aCheck)
            throws Exception {
        List<Map<String, Object>> res = localDS.executeQueryListResult(DB_CONTEXT,
                "SELECT SCENARIOS_IDS FROM CALCULATION WHERE ID=?", new Object[] {calcID});
        ...
        if (!res.isEmpty()) {
            ...
            if (doUpdate) {
                localDS.executeUpdate(DB_CONTEXT,
                        "UPDATE CALCULATION SET SCENARIOS_IDS=? WHERE ID=?", ids, calcID);
            }
        }
    ...
    public List<CalculationTable> getCalculateTableListByID(Integer aID, boolean aEnable)
            throws Exception {
        List<CalculationTable> res = getResultListObject(CalculationTable.class,
                "SELECT  S.NPP SCENARIOS_NPP, C.NAME, C.SIGN, C.SAFETY_GROUP_ID, replace(CC.FI,',','.') FI, CT.ID, CT.CALCULATION_ID, CT.SCENARIOS_ID, CT.CRITERIA_ID, CT.RANG, CT.KZNI, CT.UTCHFI, CT.G, CT.D, CT.E1, CT.E2, CT.E3 FROM SCENARIOS AS S, CALCULATION_CRITERIA AS CC, CRITERIA AS C, CALCULATION_TABLE AS CT WHERE C.ID = CT.CRITERIA_ID AND S.ID = CT.SCENARIOS_ID AND CC.CALCULATION_ID = CT.CALCULATION_ID AND CC.CRITERIA_ID = CT.CRITERIA_ID AND CT.CALCULATION_ID = ?"
                        + ((aEnable) ? " AND CT.ENABLE = 1" : "") + " ORDER BY CT.SCENARIOS_ID",
                aID);
    ...
    public void setCalculateTableEnableByID(int aID, boolean aEnable) throws Exception {
        localDS.executeUpdate(DB_CONTEXT, "UPDATE CALCULATION_TABLE SET ENABLE=? WHERE ID=?",
                (aEnable) ? 1 : 0, aID);
    }
    ...
    public void createCalculateTable(CalculationTable aCt) throws Exception {
        ...
        localDS.executeUpdate(DB_CONTEXT,
                ...
                aCt.getCALCULATION_ID(), aCt.getSCENARIOS_ID(), aCt.getCRITERIA_ID());
    }

    @Override
    public void deleteCalculateTable(int aID) throws Exception {
        localDS.executeUpdate(DB_CONTEXT, "DELETE FROM CALCULATION_TABLE WHERE ID=?", aID);

    }

    @Override
    public void updateCalculateTableID(final List<CalculationTable> calculateTable)
            throws Exception {
        @SuppressWarnings("serial")
        List<Object[]> aListParams = new ArrayList<Object[]>(calculateTable.size()) {

            {
                for (CalculationTable ct : calculateTable) {
                    add(new Object[] {ct.getKZNI(), ct.getUTCHFI(), ct.getG(), ct.getD(),
                            ct.getE1(), ct.getE2(), ct.getE3(), ct.getID()});
                }
            }
        };
        localDS.executeListUpdate(DB_CONTEXT,
