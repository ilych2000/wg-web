 ...
 * <p>
 * В пути к БД можно использовать подстановку <b>${localDB.dir}</b> она
 * указывает на папку <i>WEB-INF/classes</i> в вэб приложении.
 */
public class SQLiteDataSourceImpl implements DataSourceInt {

	/** Логер */
	private static final Logger LOG = Logger
			.getLogger(SQLiteDataSourceImpl.class);
	...
	public SQLiteDataSourceImpl(List<String> aUrls, Properties aProperties) {
		...
		String selfPath = this.getClass().getCanonicalName().replace(".", "/")
				+ ".class";
		String selfPath1 = getClass().getClassLoader().getResource(selfPath)
				.getPath();
		String dbPath = (selfPath1.substring(0,
				selfPath1.length() - selfPath.length())).replaceAll(
				"(classes|lib).+|file:", "") + "classes/";
	 ...
	 */
	@Override
	public ResultSet executeQuery(int aContext, String aSqltext,
	 ...
	 */
	@Override
	public ResultSet executeQuery(int aContext, int maxRows, String aSqltext,
			Object... aParams) throws Exception {
		Connection conn = null;
		try {
			if (LOG.isDebugEnabled()) {
				LOG.debug("Execute:"
						+ getLogingString(aContext, aSqltext, aParams));
			}
	 ...
	 * @return строка
	 */
	private String getLogingString(int aContext, String aSqltext,
			Object[] aParams) {
		return " url="
				+ ((SQLiteDataSource) _dataSourceCache.get(aContext)).getUrl()
				+ "\n\tSQL:" + aSqltext + "\n\tParameters: "
				+ StringsToString(aParams);
	}
	...
	public int executeUpdate(int aContext, String aSqltext, Object... aParams)
		...
		try {
			...
			if (ret > 0) {
				ret = conn.prepareStatement("select last_insert_rowid()")
						.executeQuery().getInt(1);
				if (LOG.isDebugEnabled()) {
					LOG.debug("Update SQL result:"
							+ getLogingString(aContext, aSqltext, aParams)
	...
	public int executeListUpdate(int aContext, String aSqltext,
		...
		try {
			...
			for (Object[] param : aListParams) {
				if (LOG.isDebugEnabled()) {
					LOG.debug("Update :"
							+ getLogingString(aContext, aSqltext, param));
				}
