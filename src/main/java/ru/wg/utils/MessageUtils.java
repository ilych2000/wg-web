...
public class MessageUtils {
	...
	/** Поле класса */
	private static final Map<Class<?>, TransParser> PARSING_MAP = new HashMap<Class<?>, MessageUtils.TransParser>() {
...

		{
			...
			TransParser dateParser = new TransParser() {
				 ...
				 */
				@Override
				public Date parse(Object value) throws ParseException,
					...
					if (value instanceof XMLGregorianCalendar) {
						return DateUtils
								.getDateFomXMLGregorianCalendar((XMLGregorianCalendar) value);
					}
					...
					try {
						return DateUtils
								.getDateFomXMLGregorianCalendarString(sValue);
			...
			TransParser xmlGregorianCalendarParser = new TransParser() {
				...
				public XMLGregorianCalendar parse(Object value)
					...
					try {
						return DateUtils
								.getXMLGregorianCalendarByString(sValue);
					} catch (IllegalArgumentException e) {
						return DateUtils.getXMLGregorianCalendar(DateUtils
								.getDateFormatString(sValue));
					}
				}
			};
			...
			put(javax.xml.datatype.XMLGregorianCalendar.class,
					xmlGregorianCalendarParser);
		}
	};
	 ...
	 * @param object
	 *            - экземпляр объекта источника
	 * @param nameToUpperCase
	 *            использовать заглавные именя полей объекта
	 * @param prefix
	 *            префикс имени поля
	 * @return Map
	 */
	public static Map<String, Object> toParametersMap(Object object,
			boolean nameToUpperCase, String prefix) {
			...
			try {
				...
				if (o != null
					...
					if (o instanceof BigInteger) {
						...
						o = DateUtils
								.getDateFomXMLGregorianCalendar((XMLGregorianCalendar) o);
					}
				}
				ret.put(fieldName, o);
			} catch (Exception e) {
				LOG.error("toParametersMap field name '" + fieldName
						+ "' set to '" + o + "'", e);
			}
		}
		return ret;
	...
	/**
	 * Заполняет поля объекта из {@link Map} не создавая объекты полей не
	 * указаных в map.
	 * 
	 * @param map
	 *            {@link Map} источник
	 ...
	 *            объект для заполнения
	 * @param prefix
	 *            префикс имени поля - родительское поле этого объекта
	 * @param mappingParams
	 *            {@link Map} для преобразования ключа из map в путь до поля
	 *            класса
	 */
	public static void fillFromMap(Map<String, Object> map, Object object,
		...
		for (Field field : fields) {
			...
			String nameField = OBJECT_FIELD_NAMING_STRATEGY_DEFAULT.getName(
					field).toUpperCase();

			if (mappingParams != null) {
				String newNameField = mappingParams.get((prefix + nameField)
						.toUpperCase());
					...
					if (o != null) {
						...
						if (!type.isAssignableFrom(o.getClass())) {
							...
							if (type.getSimpleName().equals("byte[]")) {
								...
								try {
									o = DateUtils
											.getDateFomXMLGregorianCalendarString(value);
								} catch (IllegalArgumentException e) {
									try {
										o = DateUtils
	 ...
	 * Метод
	 */
	public static boolean fillFullObjectFromMap(Map<String, Object> map,
			Object object, String prefix, Map<String, String> mappingParams) {
		return fillFullObjectFromMap(map, object, prefix, mappingParams, null);
	}

	/**
	 * Заполняет поля объекта из {@link Map} рекурсивно с воссозданием всех
	 * полей.
	 * 
	 * @param map
	 *            {@link Map} источник
	 ...
	 *            реализация интерфейса {@link ObjectFieldNamingStrategy}
	 * @return
	 */
	public static boolean fillFullObjectFromMap(Map<String, Object> map,
			Object object, String prefix, Map<String, String> mappingParams,
		...
		for (Field field : fields) {
			...
			Class<?> type = field.getType();
			String nameMaping = (prefix + namingFieldStrategy.getName(field))
					.toUpperCase();
				...
				try {
					if (isMaped) {
						if (o == null || type.isAssignableFrom(o.getClass())) {
							...
							LOG.debug("Field " + nameMaping + " is class '"
									+ type.getSimpleName()
									+ "' and not Assignable to '"
					...
					boolean isFillChanged = fillFullObjectFromMap(map,
							newObject, nameMaping, mappingParams,
							namingFieldStrategy);
					...
					LOG.error("Create Instance of " + nameMaping + "("
							+ type.getSimpleName() + ")" + ". Message: " + e);
					continue;
				}
			} else if (isMaped) {
				o = getParseObject(o, type);
	...
	/**
	 * Проверка типа класса на возможность парсить методом
	 * {@link #getParseObject}
	 * 
	 * @param type
	 *            тип класса
		...
		if (PARSING_MAP.containsKey(type)) {
			try {
				...
				if (LOG.isDebugEnabled()) {
					...
					LOG.error("parse '" + o + "' to " + type.getSimpleName()
							+ " : " + e);
				}
			}
		} else if (type.isEnum()) {
	 ...
	 * @throws Exception
	 */
	public static Object del_getTypedObject(Object oldObject, Class<?> type)
			throws Exception {
		if (oldObject == null) {
			return oldObject;
		}
	...
	/**
	 * Интерфейс для различных реализаций алгоритма получения имени поля из
	 * класса {@link Field}.
	 */
	public static abstract class ObjectFieldNamingStrategy {
		...
		/**
		 * Возвращает имя поля класса из метода {@link #getName(Field, String)}
		 * где значение по умолчанию {@link Field#getName()}
		 * 
		 * @param field
		 *            {@link Field}
		 * @return имя
	...
	/**
	 * Реализация интерфейса {@link ObjectFieldNamingStrategy} - получение имени
	 * поля по умолчанию {@link Field#getName()}
	 */
	public static ObjectFieldNamingStrategy OBJECT_FIELD_NAMING_STRATEGY_DEFAULT = new ObjectFieldNamingStrategy() {

		/**
		 * Возвращает имя по умолчанию <b>defaultName</b> или
		 * {@link Field#getName()}
		 * 
		 * @param field
		 *            {@link Field}
